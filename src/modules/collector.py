...
class Collector:
    ...
    def _from_dcm_to_png(self, directory: Path):
        ...
        self.cstm_logger.log(f"Datacube shape: {loaded_data_normalised.shape}")
        return loaded_data_normalised

    def apply(self, input_directory: str):
        dir_path = Path(input_directory)
        if not dir_path.exists():
            self.cstm_logger.log(f"Input dir dn exists {input_directory}")
            exit(1)
        file_path = dir_path / "numbers.csv"
        if not file_path.exists():
            self.cstm_logger.log(f"Input features file dn exists {str(file_path)}")
            exit(1)
        try:
            with open(str(file_path), "r") as f:
                numbers = [int(x) for x in f.read().split(",")]
        except Exception as e:
            self.cstm_logger.log("Can't load info from features file")
            pprint(e)
            exit(1)
        dicom_path = dir_path / "DICOM"
        if not dicom_path.exists():
            self.cstm_logger.log(f"Input dicom dir file dn exists {str(dicom_path)}")
            exit(1)
