...
model_path = '../models/clearer/clearer_weights_u_net.h5'
...
def clearer_model_attention_u_net(size=[512, 512]):
    ...
    for i in range(deep - 1):
        ...
        merge_layer = Multiply()([cross_data.pop(), x])

        x = Conv2D(n_filt * (pow(2, deep - 2 - i)), 3, activation=activation, padding=padding,
                   kernel_initializer=kernel_initializer)(merge_layer)
        x = Conv2D(n_filt * (pow(2, deep - 2 - i)), 3, activation=activation, padding=padding,
                   kernel_initializer=kernel_initializer)(x)
        if (i == deep - 2):
